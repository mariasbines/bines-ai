...
export function FieldworkCard({ piece }: FieldworkCardProps) {
  ...
  return (
    <article
      className="bg-paper-2 border border-ink/20 rounded-sm overflow-hidden shadow-[0_1px_0_rgba(26,24,20,0.06)] motion-safe:transition-all motion-safe:duration-200 hover:-translate-y-1 hover:shadow-lg"
      style={{ ['--color-accent' as string]: accentVar(accent) } as React.CSSProperties}
    >
      <div className="bg-accent text-ink px-8 py-3 sm:px-10 font-mono text-xs uppercase tracking-[0.14em]">
        <span className="font-bold">fieldwork {idPadded}</span>
      ...
      <div className="px-8 py-8 sm:px-10 sm:py-10">
        <h2 className="font-serif font-black text-2xl sm:text-3xl leading-[1.15] tracking-tight mb-6 break-words">
          <Link
            href={`/fieldwork/${slug}`}
